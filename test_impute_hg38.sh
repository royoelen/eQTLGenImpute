#!/bin/bash

#SBATCH --time=72:00:00
#SBATCH -N 1
#SBATCH --ntasks-per-node=1
#SBATCH --mem=6G
#SBATCH --mail-type=BEGIN
#SBATCH --mail-type=END
#SBATCH --mail-type=FAIL
#SBATCH --job-name="ImputeGenotypes"

module load java-1.8.0_40
module load singularity/3.5.3
module load squashfs/4.4

nextflow_path=/gpfs/space/GI/GV/Projects/eQTLGenPhase2/tools/

# Using the hg38 reference fasta from here: https://www.biostars.org/p/492887/

${nextflow_path}/nextflow run main.nf \
--bfile /gpfs/space/GI/GV/Projects/eQTLGenPhase2/imputation/data/EGCUT_GSA_HT12v3/filtered/GSA_HT12v3_eQTL_samples \
--harmonise_genotypes true \
--ref_genome /gpfs/hpc/projects/genomic_references/annotations/GRCh37/Homo_sapiens.GRCh37.dna.primary_assembly.fa \
--target_ref2 /gpfs/space/GI/GV/Projects/eQTLGenPhase2/imputation/data/fasta/Homo_sapiens_assembly38.fasta \
--ref_panel_hg19 /gpfs/hpc/projects/genomic_references/1000G/GRCh37/1000G_GRCh37_variant_information \
--ref_panel_hg38 /gpfs/space/GI/GV/Projects/eQTLGenPhase2/imputation/data/phasing_and_imputation_references/harmonizing_reference/30x-GRCh38_NoSamplesSorted \
--dbSNP_hg19 /gpfs/space/GI/GV/Projects/eQTLGenPhase2/imputation/references/hg19/00-All \
--dbSNP_hg38 /gpfs/space/GI/GV/Projects/eQTLGenPhase2/imputation/references/hg38/00-All \
--eagle_genetic_map /gpfs/space/GI/GV/Projects/eQTLGenPhase2/imputation/data/Eagle_genetic_map/genetic_map_hg38_withX.txt.gz \
--minimac_imputation_reference /gpfs/space/GI/GV/Projects/eQTLGenPhase2/imputation/data/phasing_and_imputation_references/imputation_reference/ \
--eagle_phasing_reference /gpfs/space/GI/GV/Projects/eQTLGenPhase2/imputation/data/phasing_and_imputation_references/phasing_reference/ \
--output_name EGCUT_GSA_HT12v3_1000Gp3HighCov_imputed \
--outdir /gpfs/space/GI/GV/Projects/eQTLGenPhase2/imputation/data/EGCUT_GSA_HT12v3_hg38_imputed_fixed \
-profile eqtlgen \
-resume